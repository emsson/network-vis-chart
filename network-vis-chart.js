...
define(["jquery", "qlik", "./scripts/vis.min"], function($, qlik, vis) {
  var cssFiles = [["vis", "vis.min.css"]];
  ...
  return {
    ...
    paint: function($element, layout) {
      var _this = this,
        app = qlik.currApp();
      (qData = layout.qHyperCube.qDataPages[0]), (id =
        layout.qInfo.qId), (containerId = "network-container_" + id);

      if (qData && qData.qMatrix) {
        $element.empty().append(
          $("<div />").attr({ id: containerId }).css({
        ...
        network.on("select", function(properties) {
          ...
          if (properties.hasOwnProperty("nodes")) {
            if (properties.nodes.length > 0) {
              ...
              var connectedNodes = network.getConnectedNodes(
                properties.nodes[0]
              );
